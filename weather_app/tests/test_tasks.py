...
def test_send_weather_notifications(mock_fetch, mock_notify, test_user, test_subscription):
    """
    Проверяет, что таска вызывает fetch_weather и notify_subscriber
    """
    # Настраиваем ответ от OpenWeather API
    ...
    # Проверка, что погода была получена
    mock_fetch.assert_called_once_with(test_subscription.city)

    # Проверка, что отправка уведомления была вызвана
